<template>
  <n-config-provider :theme="theme">
    <div class="container">
      ...
      <div class="content">
        111
        <n-button @click="openLive2dWindow" type="info">Default</n-button>
      </div>
    </div>
    <n-global-style />
  </n-config-provider>
...
async function openLive2dWindow() {
  ...
  const existingWindow = await WebviewWindow.getByLabel('live2d');

  if (existingWindow) {
    await existingWindow.show();
    await existingWindow.setFocus();
  } else {
    const live2d = new WebviewWindow('live2d', {
      url: 'live2d.html',
    });

    await live2d.once('tauri://created', function () {
      // webview successfully created
    });

    await live2d.once('tauri://error', function (e) {
